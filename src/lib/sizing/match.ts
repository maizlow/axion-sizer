...
const INERTIA_LIMIT: Record<MotorKind, number> = {
  cm3c: 15,
  cm3p: 8,
};
...
export function matchDrives(result: SizingResult, filters: MatchFilters): MatchScore[] {
  ...
  for (const motor of motors) {
    for (const gb of boxes) {
      ...
      if (!inertiaOk) {
        reasons.push(`Inertia ratio ${inertiaRatio.toFixed(1)} is high for ${motor.series}`);
      }
      if (motor.kind === "cm3c") {
        reasons.push("CM3C medium inertia — preferred when the reflected load is high");
      } else {
        reasons.push("CM3P high dynamic — preferred for short cycle times and low load inertia");
      }
