    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <h1 className="truncate text-xs font-medium tracking-tight sm:text-base">{t("caliper.tag")}</h1>
              </div>
            </div>
          </div>
      ...
      <main className="mx-auto flex max-w-[1100px] flex-col gap-6 px-4 py-6 sm:px-6">
        <aside className="rounded-[var(--radius-md)] border border-warn/40 bg-warn/10 px-4 py-3 text-sm" role="status">
          <p className="font-medium">{t("caliper.dead")}</p>
          <p className="mt-1 text-muted-foreground">{t("caliper.deadBody")}</p>
        </aside>

        <section className="rounded-[var(--radius-lg)] border border-dashed border-border bg-card p-4 sm:p-5">
          <h2 className="text-sm font-medium">{t("caliper.idea")}</h2>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">{t("caliper.ideaBody")}</p>
          <ol className="mt-3 list-decimal space-y-1 pl-5 text-sm text-muted-foreground">
            <li>{t("caliper.step1")}</li>
            <li>{t("caliper.step2")}</li>
            <li>{t("caliper.step3")}</li>
            <li>{t("caliper.step4")}</li>
          </ol>
        </section>
        ...
        <section className="pointer-events-none select-none rounded-[var(--radius-lg)] border border-border bg-card p-4 opacity-55 sm:p-5">
          <h2 className="text-sm font-medium">{t("caliper.upload")}</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="flex h-28 items-center justify-center rounded-[var(--radius-md)] border border-dashed border-border text-sm text-muted-foreground">
              {t("caliper.drop")}
            </div>
            <label className="flex flex-col gap-1 text-xs text-muted-foreground">
              {t("caliper.kind")}
              <select disabled className="h-10 rounded-[var(--radius-sm)] border border-border bg-input px-3 text-sm">
                <option>{t("caliper.kindMach")}</option>
                <option>{t("caliper.kindFab")}</option>
                <option>{t("caliper.kindLayout")}</option>
              </select>
            </label>
          </div>
          <button
            type="button"
            disabled
            className="mt-4 h-9 rounded-[var(--radius-sm)] border border-border px-3 text-sm text-muted-foreground"
          >
            {t("caliper.run")}
          </button>
        </section>
