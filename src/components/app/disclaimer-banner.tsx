import { useT } from "@/lib/i18n/locale";

const WORKBENCH = "https://www.sew-eurodrive.com/software-and-engineering/engineering-software/movisuite-and-workbench";
const CM3 = "https://www.sew-eurodrive.com/products/motors/servomotors";

export function DisclaimerBanner() {
  const t = useT();
  return (
    <aside className="border-b border-warn/30 bg-warn/10 text-sm text-foreground" role="note">
      <div className="mx-auto max-w-[1400px] px-4 py-2.5 sm:px-6">
        <p>
          <span className="font-medium">{t("disclaimer.lead")}</span> {t("disclaimer.body")}{" "}
          <a href={WORKBENCH} className="underline underline-offset-2" target="_blank" rel="noreferrer">
            {t("disclaimer.workbench")}
          </a>{" "}
          {t("disclaimer.and")}{" "}
          <a href={CM3} className="underline underline-offset-2" target="_blank" rel="noreferrer">
            {t("disclaimer.catalogs")}
          </a>{" "}
          {t("disclaimer.before")}
        </p>
      </div>
    </aside>
  );
}
